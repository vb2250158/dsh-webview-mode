...
独立插件，在当前对话中嵌入真实 iframe 网页并保留聊天区域。不创建独立自动化浏览器，不修改官方源码。插件版本为 0.2.4，配套扩展版本为 0.2.2；源码、已安装插件、运行中的 Host 和浏览器扩展版本需要分别核对。
...
| 现象 | 含义 | 动作 |
| --- | --- | --- |
| 面板报 `Extension context invalidated` | 扩展已重载，但**页面里的旧脚本已失效** | **刷新页面**（F5） |
| 面板报 `扩展为 0.2.1，需要 0.2.2` | Chrome 加载的是**旧 manifest** | **在 `chrome://extensions` 重载扩展** |
