  ...
  factory(require) {
    ...
    function Setup() {
      return h('section', { style: { padding: 20 } }, h('h3', null, '内嵌网页'),
        h('p', null, '浏览网页无需扩展；页面信息、Agent 操作及用户新标签链接需要配套扩展 0.2.1。'),
        h('p', null, '更新扩展后，请在 chrome://extensions 中重新加载 DSH WebView Mode，再刷新 DSH。部分网站禁止内嵌或限制第三方登录，可使用“在 Chrome 打开”。Agent 可读取并操作当前内嵌网页的 DOM。请使用扩展 0.2.1，在扩展选项里添加本页来源后刷新 DSH；不支持 Canvas、嵌套框架和可信物理按键。'))
    }
    ...
    function Panel({ sessionId, service, close, ready }) {
      ...
      return h('aside', { 'aria-label': '对话浏览器', style: { width: '100%', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 0, padding: 0, minHeight: 0, background: 'var(--dsw-alias-bg-layer-1)', color: 'var(--dsw-alias-label-primary)', border: 0, borderRadius: 0 } },
        ...
        h('small', null, '网页空白或拒绝连接？该站点可能禁止内嵌，请使用“在 Chrome 打开”。新标签无响应时，请重新加载扩展 0.2.1 并刷新 DSH。'),
      )
    }
    const jobs = new Map()
