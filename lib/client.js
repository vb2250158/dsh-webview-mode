        ...
        : extension.error ? `未检测到扩展响应（${extension.error}）`
          : extension.protocol !== 1 ? '扩展协议不匹配，请更新扩展'
            : !extension.configured ? '扩展已安装，但本页来源尚未授权'
              : extension.version === extensionVersion ? `已连接（${extension.version}）`
                : `已连接，但扩展为 ${extension.version}，需要 ${extensionVersion}`
      const managementUrl = extension?.id ? `chrome://extensions/?id=${extension.id}` : 'chrome://extensions'
