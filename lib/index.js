...
export const Config = z.object({
  ...
  connectionTimeoutMs: z.number().min(500).max(4000).default(3000),
...
class WebviewArchive extends TypertRemoteService {
  ...
  async setup() {
    return { extensionDirectory: fileURLToPath(new URL('../extension/', import.meta.url)), connectionTimeoutMs: this.connectionTimeoutMs }
  }
  async read(request) {
    if (!this.sessions.get(request.sessionId)) throw new Error('当前对话尚未加载')
