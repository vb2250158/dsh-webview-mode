...
export const TYPERT = {
  ...
  invocations: ['read', 'write', 'setup', 'next', 'complete', 'visibility'].map(method => ({
    ...
    result: { mode: 'strict', typeSymbol: `${packageName}#${method === 'setup' ? 'Setup' : 'Archive'}`, schema: ['next', 'complete'].includes(method) ? z.unknown() : method === 'setup' ? z.object({ extensionDirectory: z.string() }) : archiveSchema },
