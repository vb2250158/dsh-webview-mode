...
export const TYPERT = {
  ...
  invocations: ['read', 'write', 'setup', 'next', 'complete', 'visibility'].map(method => ({
    ...
    parameters: [{ name: 'request', wire: 'request', source: 'json', codec: {
      ...
      schema: method === 'visibility' ? z.object({ sessionId: z.string().min(1).max(256), open: z.boolean() }) : method === 'next' ? z.object({ sessionId: z.string(), token: z.string().optional() }) : method === 'complete' ? z.object({ sessionId: z.string(), token: z.string(), text: z.string().max(100000), error: z.string().max(4096).optional() }) : method === 'setup' ? z.object({}) : method === 'read' ? z.object({ sessionId: z.string().min(1).max(256) }) : archiveSchema,
    } }],
    result: { mode: 'strict', typeSymbol: `${packageName}#${method === 'setup' ? 'Setup' : 'Archive'}`, schema: ['next', 'complete'].includes(method) ? z.unknown() : method === 'setup' ? z.object({ extensionDirectory: z.string(), connectionTimeoutMs: z.number().int().min(500).max(4000) }) : archiveSchema },
